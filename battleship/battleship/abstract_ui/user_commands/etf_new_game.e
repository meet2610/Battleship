...
feature -- command
	...
	local
    	...
    	do
			...
			if (level ~ 13) then
				new_ships:= model.generate_ships (false,4, 2)
			elseif (level ~ 14) then
				new_ships:= model.generate_ships (false,6, 3)
			elseif (level ~ 15) then
				new_ships:= model.generate_ships (false,8, 5)
			elseif (level ~ 16) then
				new_ships:= model.generate_ships (false,12, 7)
